...
class Conv_Capsule(layers.Layer):
    ...
    def __init__(self, kernel_shape, strides, dim_vector,
                 num_routing=3, batchsize=10, name='conv_caps',
    ...
    def build(self, input_shape):
        assert len(
            input_shape) >= 5, "The input Tensor should have shape=[None, height, width, input_num_capsule, input_dim_vector]"
    ...
    def call(self, inputs, training=None):
        ...
        inputs_poses = K.reshape(inputs_poses,
                                 shape=[-1, self.kernel_shape[0] * self.kernel_shape[1] * self.kernel_shape[2],
                                        i_pose_dim])
    ...
    def capsules_dynamic_routing(self, votes):
        ...
        for i in range(self.num_routing):
            ...
            if i == self.num_routing - 1:
                ...
                self.bias += tf.reduce_sum(votes * outputs, -1, keepdims=True)
...
def PrimaryCap1(inputs, dim_vector, n_channels, kernel_size, strides, padding):
    ...
    outputs = layers.Reshape(target_shape=[shape[1], shape[2], n_channels, dim_vector], name='primarycap_reshape')(
        output)
...
def PrimaryCap2(inputs, dim_vector, n_channels, kernel_size, strides, padding):
    ...
    outputs = layers.Reshape(target_shape=[shape[1], shape[2], n_channels, dim_vector], name='primarycap_reshape_1')(
        output)
...
def ecanet_layer(x, out_dim):
    ...
    squeeze = layers.Reshape(target_shape=[shape[1], 1])(squeeze)
